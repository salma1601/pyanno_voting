...
def labels_count(annotations, nclasses, mv=MISSING_VALUE):
...
def majority_vote(annotations, mv=MISSING_VALUE):
...
def labels_frequency(annotations, nclasses):
    ...
    freq = labels_count(annotations, nclasses)
    total = sum(freq)
    #freq = [x/float(total) for x in freq]
    #return freq
    return np.array(freq) / float(total)
